...
const { fetchAllPayments } = require('./razorpay-service');

/**
 * Maps Razorpay payment object to database schema
...
function mapPaymentToSchema(payment) {
    return {
        ...
        amount: payment.amount / 100, // Convert to main currency unit if needed, assuming amount is in paise
        ...
        captured_at: payment.captured ? new Date(payment.created_at * 1000).toISOString() : null, // Razorpay doesn't always send captured_at in list, fallback to created_at or null
        raw_payload: payment,
    ...
    try {
        while (hasMore) {
            ...
            if (error) {
                ...
                // Continue or break based on severity - here we verify if it is partial failure or total
